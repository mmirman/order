...
In the calculus of constructions, this is unfortunately not an actual type. 
...
This is possible in $CC$ extended with a type heirarchy.
In this case, $\m{kind}$ would always refer to the next universe after the highest
universe mentioned in the program.
Because the visible language of Caledon does not yet have multiple universes and only implemnts
the simple $CC$, we are free to define $\m{kind}$ as $\m{type}_1$, since it will always be larger than any
